...
size_t Checkpoint::getNumMetaItems() const {
    // Sanity check - note that we ignore queue_op::empty items when
    // counting meta-items.
    auto actual_meta_items =
            std::accumulate(begin(), end(), 0,
                            [](size_t a, const queued_item& b){
        return a +
                ((b->isCheckPointMetaItem() &&
                  b->getOperation() != queue_op::empty) ? 1 : 0);
    });

    if (checkpointState == CHECKPOINT_OPEN) {
        cb_assert(1 == actual_meta_items);
        return 1;
    } else {
        cb_assert(2 == actual_meta_items);
        return 2;
    }
}
