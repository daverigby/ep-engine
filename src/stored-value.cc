...
HashTable::pauseResumeVisit(PauseResumeHashTableVisitor& visitor,
    ...
    int lock = (start_pos.lock < n_locks) ? start_pos.lock : 0;
    int hash_bucket = 0;

    for (; isActive() && !paused && lock < static_cast<int>(n_locks);
         lock++) {
        ...
        if (start_pos.lock == lock &&
            ...
            start_pos.hash_bucket < static_cast<int>(size)) {
        ...
        for (; !paused && hash_bucket < static_cast<int>(size);
             hash_bucket += n_locks) {
