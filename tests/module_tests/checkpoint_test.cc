...
TEST_F(CheckpointTest, reset_checkpoint_id) {
    ...
    manager->checkAndAddNewCheckpoint(1, *vbucket.get());
...
TEST_F(CheckpointTest, CursorOffsetOnCheckpointClose) {
    ...
    EXPECT_EQ(2, manager->removeClosedUnrefCheckpoints(*vbucket.get(),
...
TEST_F(CheckpointTest, CursorMovement) {
    ...
    bool newCheckpointCreated;
    manager->removeClosedUnrefCheckpoints(*vbucket.get(), newCheckpointCreated);
