...
static enum test_result test_defragmenter(ENGINE_HANDLE *h,
    ...
    // less than 80% of the amount before defrag (this is pretty conservative,
    // but it's hard to accurately predict the whole-applicaiton size).
    // Give it 30 seconds to drop.
    const size_t expected_mapped = ((mapped_2 - mapped_0) * 0.8) + mapped_0;
...
engine_test_t* get_tests(void) {
    ...
    TestCase tc[] = {
        ...
        TestCase("test defragmenter", test_defragmenter,
                 test_setup, teardown, "defragmenter_interval=9999", prepare,
                 cleanup),
